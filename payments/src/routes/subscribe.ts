...
router.post(
    "/api/payments/subscribe/:subscriptionId",
    requireAuth,
    catchAsync(async (req: Request, res: Response) => {
        ...
        console.log("active subscription", subscriptions);
        if (subscriptions.data.length) {
            const stripeSession = await stripe.billingPortal.sessions.create({
                customer: customer.id,
                return_url: `${process.env.CORS_ORIGIN!}/subscription`,
            });
            throw new subscriptionError("User already Subscribed to a plan", {
        ...
        if (!subscription) {
            throw new BadRequestError(
                "No Subcription with given Subscription ID"
            );
        }

        let tenure: Date = new Date();
        const date = new Date();

        if (subscription.billingPeriod === "week") {
            tenure = new Date(
                date.getFullYear(),
                date.getMonth(),
                date.getDay() + 7
