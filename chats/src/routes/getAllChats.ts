...
router.get('/api/chats/getAll',requireAuth,catchAsync(async(req:Request,res:Response)=>{
    ...
    const chats = await Conversation.aggregate([
      {
        $match: {
          members: { $in: [new mongoose.Types.ObjectId(req.currentUser._id)] },
        },
      },
      {
        $lookup: {
                ...
                $arrayElemAt: [
                  {
                    $filter: {
                      input: "$participants",
                      cond: { $ne: ["$$this._id", new mongoose.Types.ObjectId(req.currentUser._id)] }
                    }
                  }, 0
                ]
              },
