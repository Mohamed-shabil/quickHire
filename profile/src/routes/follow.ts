...
router.post(
    "/api/profile/followers/follow/:userId",
    requireAuth,
...
router.delete(
    "/api/profile/followers/unfollow/:userId",
    requireAuth,
    catchAsync(async (req: Request, res: Response) => {
        const { userId } = req.params;
        ...
        if (!isFollowing) {
            throw new BadRequestError(
                "You are not Following the User to unfollow"
            );
        }
        const unFollow = await Follow.findOneAndDelete(
            { follow: userId, followedBy: req.currentUser?._id },
            { new: true }
        );
...
router.get(
    "/api/profile/followers/getFollowings",
    requireAuth,
    [
        body("userId").notEmpty().isMongoId().withMessage("Invalid Mongo ID"),
        validateRequest,
    ],
    catchAsync(async (req: Request, res: Response) => {
...
router.get(
    ...
    catchAsync(async (req: Request, res: Response) => {
        ...
        const followers = await Follow.find({ follow: userId }).populate(
            "follow"
        );
