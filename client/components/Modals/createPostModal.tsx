...
import { Check, Laugh, Loader2, Router,Image as ImageIcon, X } from "lucide-react"
          ...
          <form onSubmit={onSubmit} className="mt-8 grid grid-cols-6 gap-6">
            <div className="col-span-6">
              <FormField
                ...
                render={({field})=>(
                  <FormItem>
                    <div className="col-span-6">
                      <Textarea {...field} value={caption} className="border-0 focus-visible:ring-white focus-visible:ring-0 " placeholder="What's in your mind..." onChange={(e)=>{setCaption(e.target.value)}}/>
                    </div>
                  </FormItem>
              )}/>
            </div>
            {preview ? 
              <div className="col-span-6 relative">
                <X className="absolute right-2 top-2 cursor-pointer text-slate-700 hover:text-black transition-all h-4 w-" onClick={()=>setPreview('')}/>
                <Image src={preview} alt='uploading File' width={350} height={350} className="col-span-6 rounded-md w-full h-80 object-cover"/>
              </div>:
              <></>
            }
                  ...
                  <FormField
                    ...
                    render={({field})=>(
                      <FormItem className="flex flex-1 w-9">
                          <Label htmlFor="upload" className="p-2 border hover:bg-blue-500/10 transition-all rounded">
