...
import React, { Ref, RefObject, useContext, useEffect, useRef, useState } from "react";
    ...
    const callUser:IPeerContext['callUser'] = (id: string) => {
      ...
      console.log('Calling User ,,,,,,');
        ...
        peer.on('signal', (data) => {
          ...
          console.log(id);
          socket?.emit('calluser', { userToCall: id, signalData: data, from: currentUser?._id,});
        });
      ...
      {call?.isReceivingCall && !callEnded &&(
        toast({
          ...
          action: (
            <Link href={'/chats/videoCall'}>Answer</Link>
          ),
        })
      )}
    return(
